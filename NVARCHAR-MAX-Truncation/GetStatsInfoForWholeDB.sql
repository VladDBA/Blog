...
SELECT @SQL = N'SELECT DB_NAME() AS [database],'
+ @LineFeed + N'SCHEMA_NAME([obj].[schema_id]) + N''.'''
...
+ @LineFeed + N', N''UPDATE STATISTICS ''+QUOTENAME(DB_NAME())+N''.''+QUOTENAME(SCHEMA_NAME([obj].[schema_id]))+N''.'''
+N'+ QUOTENAME([obj].[name]) +N'' /*WITH FULLSCAN*/ /*WITH SAMPLE ? PERCENT*/;'' AS [table_stats_update_command]'
