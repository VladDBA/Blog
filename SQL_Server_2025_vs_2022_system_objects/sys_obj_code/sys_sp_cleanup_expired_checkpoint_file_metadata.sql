...
CREATE PROCEDURE sys.sp_cleanup_expired_checkpoint_file_metadata
...
BEGIN
	...
	-- delete from sys.manageddeltatablecheckpoints. We always keep the oldest checkpoint below the GC expiration_time threshold.
	DELETE TOP (@batch_size) FROM sys. manageddeltatablecheckpoints 
	WHERE table_id = @table_id AND delta_log_commit_sequence_id < @max_delta_log_commit_sequence_id_exclusive;
