...
/*
** Check if this is DW Fido database, exec the sp_fido_spaceused and return the result.
** We should only have record in sys.dm_dw_databases, when Fido is enabled.
*/
IF (HAS_PERMS_BY_NAME(null, 'DATABASE', 'VIEW DATABASE STATE') = 1)
BEGIN
	IF EXISTS (SELECT * FROM sys.dm_dw_databases where logical_db_name = db_name())
	BEGIN
		IF @objname IS NOT NULL
			SET @remotesql = CONCAT(
					N'sys.sp_fido_spaceused  @objname = N''',
					@objname,
					N''' , @updateusage = ',
					@updateusage,
					N' , @oneresultset = ',
		...
		BEGIN CATCH
			...
			raiserror(22453, 16, 1, @errornumber, @errorseverity, @errorstate, @errorline, @errormessage)
			return (1)
		END CATCH
	END
END
